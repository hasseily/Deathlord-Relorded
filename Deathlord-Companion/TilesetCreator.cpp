...
char* TilesetCreator::parseTilesInHGR2()
{
    ...
    */
    ...
    for (UINT32 iT = 0; iT < (tileByteSize * 0x80); iT += tileByteSize)  // Tile by tile, stop at 128 tiles (the rest is garbage)
    {
		...
		UINT32 iFBOriginByte = (TOPMARGIN * FRAMEBUFFERWIDTHB) + (LEFTMARGIN * PIXELDEPTH);   // The tile is always the first tile on the top left
		...
		UINT32 iPNGOriginByte = (iPNGRow * PNGTH * PNGBUFFERWIDTHB) + ((UINT32)iPNGCol * PNGTW * PIXELDEPTH);
