...
std::wstring YamlLoadHelper::LoadWString(const std::string& key)
{
	std::string value = LoadString(key);
...
void YamlSaveHelper::SaveWString(const char* key, const std::wstring& wvalue)
{
	SaveString(key,std::wstring_convert<std::codecvt_utf8<wchar_t>>().to_bytes(wvalue));
}
