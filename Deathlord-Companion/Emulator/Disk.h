...
public:
	std::wstring m_imagename;	// <FILENAME> (ie. no extension)
	std::wstring m_fullname;	// <FILENAME.EXT> or <FILENAME.zip>  : This is persisted to the snapshot file
	std::wstring m_strFilenameInZip;					// ""             or <FILENAME.EXT>
...
public:
	...
	const std::wstring & GetFullDiskFilename(const int drive);
	const std::wstring & GetFullName(const int drive);
	const std::wstring & GetBaseName(const int drive);
	...
	UINT GetCurrentFirmware(void) { return 16; }
	...
	bool UserSelectNewDiskImage(const int drive, std::wstring* wFilename, LPCWSTR sTitle = L"Select Image");
...
private:
	...
	const std::wstring & DiskGetFullPathName(const int drive);
