...
static DWORD Cpu65C02(DWORD uTotalCycles, const bool bVideoUpdate)
{
	...
	do
	{
		...
		if (g_isInBattle)
		{
			...
			// If when in battle none of these switches trigger,
			// fallback to the g_inInGameMap switch.
			switch (_origPC)
			{
			...
			{
				if (!g_nonVolatile.relordedChanges.xp_reallocation)
					break;
				...
				// [Total battle XP is at 0xAFEE(LO)-AFEF(HI)]
				...
				// this LDX #$00 starts the XP alloc routine at char 0 every time, after it's gone through allocating
				// 1 XP to each char.
				// We'll let the game allocate all the XP except for the overflow, which we'll allocate
				...
				if (_battleXP < DEATHLORD_PARTY_SIZE) // overflow, some members won't get XP
				{
					...
					while (_battleXP > 1)	// allocate every time 1 XP to a random char
					{
						...
						UINT8 _xpLo = MemGetMainPtr(PARTY_XP_LOBYTE_START)[_currCharForXP];
						if (_xpLo == UINT8_MAX)
...
SWITCH_GAMEMAP:
			...
			case PC_MAP_KEY_PRESS:
			{
				...
				if (g_nonVolatile.relordedChanges.exit_pit_by_moving)
				{
			...
			{
				if (!g_nonVolatile.relordedChanges.freeze_time_when_idle)
					break;
			...
			{
				if (!g_nonVolatile.relordedChanges.ranged_attack_for_rear_line)
					break;
			...
			{
				if (!g_nonVolatile.relordedChanges.search_always_succeeds)
					break;
			...
			{
				if (!g_nonVolatile.relordedChanges.no_level_drain)
					break;
			...
			{
				if (!g_nonVolatile.relordedChanges.magic_water_increases_stats)
					break;
			...
			{
				if (g_nonVolatile.relordedChanges.no_stats_limit)
				{
					CYC(2); // BCS uses 2 cycles;
					regs.pc = _origPC + 2;	// Jump to the next instruction, disregard the branch
				}
			...
			{
				if (!g_nonVolatile.relordedChanges.no_hp_loss_from_starvation)
					break;
			...
			{
				if (!g_nonVolatile.relordedChanges.extra_race_and_class_bonuses)
					break;
			...
			{
				if (!g_nonVolatile.relordedChanges.no_autosave_after_death)
					break;
			...
			{
				if (!g_nonVolatile.relordedChanges.expanded_weapon_use)
					break;
			...
			{
				if (!g_nonVolatile.relordedChanges.keep_extra_xp_on_levelup)
					break;
			...
			case PC_BUY_FOOD:
			{
				if (!g_nonVolatile.relordedChanges.distribute_food)
					break;
			...
			{
				if (!g_nonVolatile.relordedChanges.fix_gold_pooling)
					break;
			...
			{
				if (!g_nonVolatile.relordedChanges.distribute_gold)
					break;
