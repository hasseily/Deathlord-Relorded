...
// NTSC_END
		...
		if (NMI(uExecutedCycles, flagc, flagn, flagv, flagz) || IRQ(uExecutedCycles, flagc, flagn, flagv, flagz))
		{
			// Allow AppleWin debugger's single-stepping to just step the pending IRQ
		}
		else
		{
			bool executeOpcode = true;
			if (g_cpuInstructionHook)
			{
				EF_TO_AF
				const CpuInstructionHookResult hook =
					g_cpuInstructionHook(regs.pc, g_cpuInstructionHookUserData);
				AF_TO_EF
				uExecutedCycles += hook.cycles;
				executeOpcode = !hook.skipOpcode;
			}
