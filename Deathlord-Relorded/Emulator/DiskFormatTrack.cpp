	...
	if (m_trackState == TS_ADDRFIELD && m_4and4idx < 8)
	{
		...
		if (m_4and4idx == 8)
		{
			...
			if (!bIsWrite)
			{
				...
				wchar_t str[100];
				wsprintf(str, L"read D5AA%02X detected - Vol:%02X Trk:%02X Sec:%02X Chk:%02X %s", addrPrologue, m_VolTrkSecChk[0], m_VolTrkSecChk[1], m_VolTrkSecChk[2], m_VolTrkSecChk[3], chk?L"":L"(bad)");
