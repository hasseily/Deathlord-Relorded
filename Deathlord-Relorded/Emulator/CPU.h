...
// Optional frontend hooks. The portable emulator deliberately knows nothing
// about any particular game; a host may observe or replace an instruction at
// its current PC and may observe keyboard-latch reads.
struct CpuInstructionHookResult
{
	uint32_t cycles = 0;
	bool skipOpcode = false;
};
typedef CpuInstructionHookResult (*CpuInstructionHookFn)(WORD pc, void* userData);
extern CpuInstructionHookFn g_cpuInstructionHook;
extern void* g_cpuInstructionHookUserData;

typedef void (*KeyboardReadHookFn)(void* userData);
extern KeyboardReadHookFn g_keyboardReadHook;
extern void* g_keyboardReadHookUserData;
