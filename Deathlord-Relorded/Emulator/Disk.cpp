	...
	m_slot(slot)
{
	...
	m_floppyLatch = 0;
	m_saveDiskImage = true;	// Save the DiskImage name to file
	m_diskLastCycle = 0;
...
std::wstring Disk2InterfaceCard::GetCurrentTrackString(void)
{
	...
	wchar_t szInt[8] = L"";
	wsprintf(szInt, L"%02X", trackInt);		// "$NN"

	wchar_t szFrac[8] = L"";
	wsprintf(szFrac, L"%.02f", trackFrac);	// "0.nn"

	return std::wstring(szInt) + std::wstring(szFrac+1);
}

std::wstring Disk2InterfaceCard::GetCurrentPhaseString(void)
{
	...
	wchar_t szInt[8] = L"";
	wsprintf(szInt, L"%02X", phaseInt);		// "$NN"

	wchar_t szFrac[8] = L"";
	wsprintf(szFrac, L"%.02f", phaseFrac);	// "0.nn"
...
void Disk2InterfaceCard::LoadLastDiskImage(const int floppy)
{
	_ASSERT(floppy == FLOPPY_BOOT || floppy == FLOPPY_SCENA || floppy == FLOPPY_SCENB);
	...
	case FLOPPY_BOOT:
	{
		InsertDisk(DRIVE_1, g_nonVolatile.diskBootPath.c_str(), IMAGE_USE_FILES_WRITE_PROTECT_STATUS, IMAGE_DONT_CREATE);
		break;
	}
	case FLOPPY_SCENA:
	{
		InsertDisk(DRIVE_1, g_nonVolatile.diskScenAPath.c_str(), IMAGE_USE_FILES_WRITE_PROTECT_STATUS, IMAGE_DONT_CREATE);
		break;
	}
	case FLOPPY_SCENB:
	{
		InsertDisk(DRIVE_2, g_nonVolatile.diskScenBPath.c_str(), IMAGE_USE_FILES_WRITE_PROTECT_STATUS, IMAGE_DONT_CREATE);
		break;
	}
	default:
		break;
...
void Disk2InterfaceCard::SaveLastDiskImage(const int floppy)
{
	...
	if (wcsrchr(szPathName, TEXT('\\')))
	{
		...
		case FLOPPY_BOOT:
		{
			g_nonVolatile.diskBootPath = szPathName;
			break;
		}
		case FLOPPY_SCENA:
		{
			g_nonVolatile.diskScenAPath = szPathName;
			break;
		}
		case FLOPPY_SCENB:
		{
			g_nonVolatile.diskScenBPath = szPathName;
			break;
		}
		default:
			break;
...
const std::wstring & Disk2InterfaceCard::GetFullDiskFilename(const int drive)
{
...
const std::wstring & Disk2InterfaceCard::GetFullName(const int drive)
{
...
const std::wstring & Disk2InterfaceCard::GetBaseName(const int drive)
{
	return m_floppyDrive[drive].m_disk.m_imagename;
}

void Disk2InterfaceCard::GetFilenameAndPathForSaveState(std::wstring& filename, std::wstring& path)
{
	filename = L"";
	path = L"";
	...
	for (UINT i=DRIVE_1; i<=DRIVE_2; i++)
	{
		...
		std::wstring pathname = DiskGetFullPathName(i);
...
const std::wstring & Disk2InterfaceCard::DiskGetFullPathName(const int drive)
{
...
ImageError_e Disk2InterfaceCard::InsertDisk(const int drive, LPCTSTR pszImageFilename, const bool bForceWriteProtected, const bool bCreateIfNecessary)
{
	...
	{
		const std::wstring & pszOtherPathname = DiskGetFullPathName(!drive);

		wchar_t szCurrentPathname[MAX_PATH]; 
		DWORD uNameLen = GetFullPathName(pszImageFilename, MAX_PATH, szCurrentPathname, NULL);
		if (uNameLen == 0 || uNameLen >= MAX_PATH)
			wcscpy_s(szCurrentPathname, MAX_PATH, pszImageFilename);

		if (!wcscmp(pszOtherPathname.c_str(), szCurrentPathname))
		{
			EjectDisk(!drive);
		}
	}
...
void Disk2InterfaceCard::DumpTrackWOZ(FloppyDisk floppy)	// pass a copy of m_floppy
{
	...
	while (1)
	{
		...
		{
			newLine = false;
			StringCbPrintf(str, sizeof(str), L"%04X:", floppy.m_bitOffset & 0xffff);
		...
		if (zeroCount == 0)	StringCbPrintf(str, sizeof(str), L"   %02X", shiftReg);
		else				StringCbPrintf(str, sizeof(str), L"(%c)%02X", syncBits, shiftReg);
		OutputDebugString(str);
		...
		if ((nibbleCount % 32) == 0)
		{
			std::wstring strReadDetected = formatTrack.GetReadD5AAxxDetectedString();
	...
	{
		char syncBits = zeroCount <= 9 ? '0'+zeroCount : '+';
		StringCbPrintf(str, sizeof(str), L"(%c)", syncBits);
		OutputDebugString(str);
	}
	...
	if (shiftReg)
	{
		StringCbPrintf(str, sizeof(str), L"%02X/Partial Nibble", shiftReg);
		OutputDebugString(str);
	}
...
bool Disk2InterfaceCard::UserSelectNewDiskImage(const int drive, std::wstring* wFilename, LPCWSTR sTitle)
{
	...
	{
		MessageBox(g_hFrameWindow, L"Drive not connected!", L"Insert disk", MB_ICONEXCLAMATION|MB_SETFOREGROUND|MB_OK);
		return false;
	}
...
bool Disk2InterfaceCard::DriveSwap(void)
{
	...
	if (m_floppyDrive[DRIVE_1].m_spinning || m_floppyDrive[DRIVE_2].m_spinning)
	{
		...
		int status = MessageBox(g_hFrameWindow, 
			L"WARNING:\n"
				"\n"
			...
			L"Trying to swap a disk while a drive is on ...",
...
{
	HRSRC hResInfo = FindResource(NULL, MAKEINTRESOURCE(lpNameId), L"FIRMWARE");
	if (hResInfo == NULL)
		return false;
